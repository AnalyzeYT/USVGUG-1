"""
YouTube Video Creator - Gradio App
This app provides a web interface for creating YouTube videos from stories
"""
import os
import re
import time
...
parser.add_argument('--device', type=str, default='cpu', choices=['cuda', 'cpu'], help='Device to use for model (cuda/cpu)')
parser.add_argument('--compute_type', type=str, default='float32', choices=['float16', 'float32'], help='Compute type for model')
...
def load_whisperx_model(device=args.device, compute_type=args.compute_type):
    """
...
def extract_timestamps(audio_file, device=args.device, compute_type=args.compute_type):
    """
...
def create_video(story, genre, title, pexels_api_key, tenor_api_key, include_subtitles=True, 
               resolution="720p", progress=gr.Progress()):
    """
    ...
    Returns:
        Path to the output video file, progress message
    """
    ...
    try:
        ...
        progress(0.1, "Converting text to speech...")
        audio_file = os.path.join(temp_dir, "audio.mp3")
        speech_result = text_to_speech(story, audio_file)
        
        if not os.path.exists(audio_file):
            return None, f"Error: {speech_result}"
        
        # Step 2: Extract word-level timestamps
        progress(0.2, "Extracting word-level timestamps...")
        word_timestamps = extract_timestamps(audio_file)
        
        if not word_timestamps:
            return None, "Error: Failed to extract timestamps"
        
        # Step 3: Group words into phrases for media search
        progress(0.3, "Analyzing text for media matching...")
        ...
        progress(0.4, "Fetching and preparing media...")
        video_clips = []
        
        for i, phrase in enumerate(phrases):
            phrase_progress = 0.4 + (0.4 * (i / len(phrases)))
            progress(phrase_progress, f"Processing phrase {i+1}/{len(phrases)}...")
        ...
        progress(0.8, "Creating final video...")
        ...
        progress(0.95, "Rendering final video...")
        ...
        end_time = time.time()
        final_message = f"Video created successfully in {end_time - start_time:.2f} seconds!"
        progress(1.0, final_message)
...
def gradio_interface(
    ...
    if not story.strip():
        return None, "Error: Please provide a story"
    
    if not title.strip():
        return None, "Error: Please provide a title"
    
    if not pexels_api_key.strip() or not tenor_api_key.strip():
        return None, "Error: Please provide both API keys"
    
    # Create the video
    output_video, message = create_video(
        story=story,
        genre=genre,
        title=title,
...
with gr.Blocks(title="YouTube Video Creator") as demo:
    gr.Markdown("# YouTube Video Creator")
    gr.Markdown("Create complete YouTube-ready videos from your stories.")
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            # Sample story button
            def load_sample():
                return """The small fishing village awakened as the morning sun painted the horizon in shades of amber and gold. 
...
the village, the market sprang to life with vendors arranging fresh produce and morning delicacies. The village 
had thrived this way for generations, finding balance between tradition and the rhythms of nature."""
            
            sample_btn = gr.Button("Load Sample Story")
            sample_btn.click(fn=load_sample, outputs=story_input)
    ...
    with gr.Accordion("Help & Information", open=False):
        gr.Markdown("""
        ## How to Use
        1. Enter your story in the text box, or click "Load Sample Story"
        2. Select a genre that matches your story
        3. Enter a title for your video
        4. Choose whether to include subtitles
        5. Select your preferred resolution (higher resolution takes longer)
        6. Enter your API keys:
           - Get a [Pexels API key](https://www.pexels.com/api/)
           - Get a [Tenor API key](https://developers.google.com/tenor/guides/quickstart)
        7. Click "Create Video" and wait for processing to complete
        
        ## About
        This app creates videos by:
        - Converting text to speech
        - Extracting word-level timestamps
        - Finding videos and GIFs that match your story
        - Adding subtitles
        - Combining everything into a complete video
        """)
